...
1. 問題無く実行できれば、us-east1 リージョンに e2-micro で Ubuntu20.04 の環境ができあがります。
	...
	* マルチゾーンに分散して作る時のために、あえて count を使いつつ、サブネットも 2個作ってあります。丸 1ヶ月稼働させることさえなければ、一時的に `count = 2` としていただくなどして、複数台の検証も可能と思われます。ただしこの場合は完全にすべてを削除したままの時間を作らないと、完全無料にできないはずなので、ご注意ください。
